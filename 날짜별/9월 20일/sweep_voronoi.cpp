...
int main() {
	...
	for (int i = 0; i < edge.size(); i++) {
		...
		for (int x = max(1, (int)ceil(p1.first)); x <= min(m, (int)floor(p2.first)); x++) {
			...
			if (y < 1) {
				bound(x, 1, area[i].first);
				bound(x, 1, area[i].second);
			}
			else if (y > m) {
				bound(x, m, area[i].first);
				bound(x, m, area[i].second);
			}
			else {
	...
	int order[1010101];
	for (int i = 0; i < n; i++) order[reorder[i].second] = i;
	for (int x = 1; x <= m; x++) {
		for (int y = 1; y <= m; y++) bound(x, y, order[grid[x][y - 1]]);
		for (int y = m; y >= 1; y--) bound(x, y, order[grid[x][y + 1]]);
	}
