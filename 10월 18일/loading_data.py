# 경로 변수 저장
# csv_path = "data/time_data_662.csv"
csv_path = "data/plot_by_n_last.csv"
...
for name in tqdm(algorithm_list):
	df_tmp=df[(df['name']==name)&(df[type_of_fixed_value]==fixed_value)&(df[type_of_graph]>=min_value)&(df[type_of_graph]<=max_value)]
	# print(df_tmp.info())
...
for df_tm in tqdm(df_list):
    df_tm[type_of_graph]=np.log(df_tm[type_of_graph])
    df_tm['time']=np.log(df_tm['time'])
